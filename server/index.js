...
passport.serializeUser((user, done) => {
    ...
    permissionForTexts:
      user._json["https://example.com/notifications"] === "true" ? true : false,
    role: user._json["https://example.com/role"] || "",
    activeStatus:
      user._json["https://example.com/status"] === "true" ? true : false
  });
});
...
app.get("/api/auth/login", passport.authenticate("auth0"), (req, res, done) => {
  ...
  dbInstance.readUser([passportUser.id]).then(user => {
    ...
    if (user.length && user[0].user_id) {
      ...
      dbInstance
        .createUser([
          ...
          passportUser.name,
          passportUser.unit,
          passportUser.complex,
          passportUser.email,
          passportUser.phone,
          passportUser.permissionForTexts,
          passportUser.activeStatus,
          passportUser.role
        ])
        .then(user => {
...
app.get("/api/user", (req, res) => {
  ...
  const { session } = req;
  dbInstance
    .readUser([session.passport.user.id])
    .then(user => {
      console.log(user[0]);
...
app.listen(
  port,
  console.log(`Stockton to Malone...connects at the buzzer!!! on port ${port}`)
);
